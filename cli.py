...
def main():
    ...
    if args.command == "fetch":
        ...
        if not runs:
            ...
            print(f"\n{'ID':<5} {'Ticker':<15} {'Strategy ID':<13} {'Return':<10} {'Sharpe':<10} {'Drawdown':<12} {'Win Rate':<10} {'Trades':<8} {'Run At'}")
            print("-" * 100)
            for r in runs:
                print(f"{r[0]:<5} {str(r[1]):<15} {str(r[2]):<13} {str(r[6])+'%':<10} {str(r[7]):<10} {str(r[8])+'%':<12} {str(r[9])+'%':<10} {str(r[10]):<8} {str(r[11])[:16]}")
