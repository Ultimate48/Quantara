...
export default function App() {
  ...
  const [selectedPreset, setSelectedPreset] = useState('SMA');
  const [presetParams, setPresetParams] = useState({ period: 20, fast: 12, slow: 26, signal: 9 });
  ...
  // Update form fields when selectedStrategy changes
  useEffect(() => {
    if (selectedStrategy) {
  ...
  // Add empty column to strategy columns
  ...
  // Insert indicator preset into columns
  const handleInsertPreset = () => {
    ...
    // De-duplicate names: if any expanded name already exists, filter it out or warn
  ...
  const handleSaveStrategy = async (e) => {
    ...
    try {
      ...
      // Select the active strategy and turn off edit mode
      const activeName = isEditingStrategy ? selectedStrategy.name : bodyData.name;
      const found = strategies.find(s => s.name === activeName) || bodyData;
  ...
  const handleDeleteStrategy = async (force = false) => {
    ...
    try {
      ...
      showToast(data.message || `Strategy '${selectedStrategy.name}' deleted.`, 'success');
      setForceDeleteTarget(null);
      
      // Reset selected strategy and reload
      setSelectedStrategy(null);
      await fetchStrategies();
      fetchBacktests(); // might have deleted some runs
    } catch (err) {
      showToast(err.message, 'error');
    }
  };
        ...
        {activeTab === 'strategies' && (
          <div>
            <header className="page-header">
              ...
              <p className="page-subtitle">Configure columns via technical indicator formulas and write signaling trigger logic.</p>
            </header>
                ...
                <form onSubmit={handleSaveStrategy}>
                  <div className="form-group">
                    ...
                    <input 
                      type="text" 
                      className="input-field" 
                      placeholder="e.g. DoubleMovingAverage"
                      value={stratName}
                  ...
                  <div className="form-group">
                    ...
                    <textarea 
                      ...
                      placeholder="e.g. 1 if sma_5 > sma_20 else (-1 if sma_5 < sma_20 else 0)"
                      value={stratSignal}
                  ...
                  {validationError && (
                    <div style={{ background: 'rgba(239, 68, 68, 0.1)', border: '1px solid var(--color-red)', padding: '1rem', borderRadius: '8px', marginBottom: '1.5rem', fontSize: '0.9rem', color: '#ff8a8a', whiteSpace: 'pre-wrap' }}>
